...
<template>
    ...
    >
        ...
        >
            <BriefcaseIcon class="size-6 stroke-1" />
        </button>
        <TheButton
            v-else
            @click="store.open = true"
    ...
    <QuickMenuSidebar
        ...
        description="Manage client services, pricing and work units"
        :icon="BriefcaseIcon"
        @close="store.open = false"
    >
        <template #title-extra>
            <span
                class="hidden rounded-2xl border border-zinc-300 bg-white/90 px-2 py-0.5 text-xs font-medium text-zinc-600 backdrop-blur-sm sm:block dark:border-zinc-700 dark:bg-zinc-900/70 dark:text-zinc-400"
            >
        ...
        <template #subheader>
            <div class="border-t border-zinc-300/70 dark:border-zinc-800/70"></div>
            ...
            <div class="flex items-center gap-3 px-3 py-3">
                ...
                <div
                    class="flex shrink-0 rounded-2xl border border-zinc-300 bg-white p-1 shadow-sm dark:border-zinc-800 dark:bg-zinc-900"
                >
                    <button
                        type="button"
                        class="rounded-2xl px-3 py-1.5 text-sm font-medium transition"
                        :class="
                            tab === 'style'
        ...
        <div class="grid h-full min-h-0 grid-cols-1 md:grid-cols-2">
            <!-- list -->
            <section
                class="overflow-y-auto border-b border-zinc-300 px-2 pb-4 [scrollbar-gutter:stable] md:border-r md:border-b-0 dark:border-zinc-800"
            >
                <div class="flex items-center justify-between gap-3 px-3 py-3">
                    <div class="text-sm text-zinc-600 dark:text-zinc-400">
                        ...
                        <span
                            ...
                            class="ml-2 text-xs text-zinc-600 dark:text-zinc-500"
                        >
                            loading…
                        </span>
                    </div>
                    ...
                    >
                        <BriefcaseIcon class="size-4 stroke-2" />
                        New
                    </TheButton>
                </div>
                    ...
                    <button
                        ...
                        :class="
                            selectedId === p.id
                                ...
                                : 'border-zinc-300 bg-white hover:bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-950/20 dark:hover:bg-zinc-950/30'
                        "
                        @click="pick(p)"
                    >
                        <div class="flex items-center justify-between gap-3">
                            <div
                                class="line-clamp-1 text-sm font-semibold text-zinc-700 dark:text-zinc-100"
                            >
                                {{ p.productName }}
                            </div>
                            <div class="text-xs whitespace-nowrap text-zinc-600 dark:text-zinc-300">
                                {{
                        ...
                        <div class="mt-1 text-xs text-zinc-600 dark:text-zinc-500">
                            {{ p.productType }} • {{ p.pricingMode }}
                            <span v-if="p.pricingMode === 'hourly'">
                                • {{ p.minutesWorked ?? '—' }} min
                            </span>
                        </div>
                    </button>
            ...
            <section class="overflow-y-auto px-6 py-4 [scrollbar-gutter:stable]">
                ...
                <div class="mt-4 space-y-4 pb-12 sm:pb-0">
                    ...
                    <div
                        v-if="form.id"
                        class="rounded-2xl border border-zinc-300/80 bg-zinc-50/80 p-3 dark:border-zinc-800 dark:bg-zinc-950/20"
                    >
                        <div
                            class="text-tiny mb-2 font-medium tracking-[0.12em] text-zinc-600 uppercase dark:text-zinc-400"
                        >
                            Record details
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-2 sm:grid-cols-2">
                            <div
                                class="rounded-xl border border-zinc-300 bg-white px-3 py-2 dark:border-zinc-800 dark:bg-zinc-950/50"
                            >
                                <div
                                    class="text-tiny font-medium tracking-wide text-zinc-600 uppercase dark:text-zinc-400"
                                >
                                    Created
                                </div>
                                <div
                                ...
                                >
                                    {{
                                        form.createdAt
                                            ? fmtDisplayDate(new Date(form.createdAt))
                                            : 'N/A'
                                    }}
                                </div>
                            </div>
                            ...
                            <div
                                class="rounded-xl border border-zinc-300 bg-white px-3 py-2 dark:border-zinc-800 dark:bg-zinc-950/60"
                            >
                                <div
                                    class="text-tiny font-medium tracking-wide text-zinc-600 uppercase dark:text-zinc-400"
                                >
                                ...
                                >
                                    {{
                                        form.updatedAt
                                            ? fmtDisplayDate(new Date(form.updatedAt))
                                            : 'N/A'
                                    }}
                                </div>
                            </div>
