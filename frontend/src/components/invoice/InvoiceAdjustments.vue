    ...
    <div class="space-y-2 divide-y divide-zinc-200 dark:divide-zinc-800">
        ...
        <section class="py-4 first:pt-0">
            <div class="mb-2 flex items-center justify-between gap-3">
                ...
                <TheTooltip side="top" align="center" class="hover:text-sky-600 dark:hover:text-emerald-400"
            ...
            <div class="flex flex-col gap-2 sm:flex-row">
                <div class="flex justify-between gap-2">
                    <TheInput v-model="discount" type="number" placeholder="0" labelHidden :reserveErrorSpace="false"
                        :disabled="discountMode === 'none'" :title="discountMode === 'none'
                            ? 'select discount mode from dropdown first'
                            : 'discount value'
                            " inputClass="w-full py-1" :error="discountError" />
                    <TheDropdown v-model="discountMode" input-class="py-1 min-w-32 sm:min-w-24"
                        :right-icon="ChevronUpDownIcon" :options="['none', 'fixed', 'percent']" />
                </div>
        ...
        <section class="py-4">
            ...
            <div class="flex flex-col gap-2 sm:flex-row">
                <div class="flex justify-between gap-2">
                    <TheInput v-model="deposit" type="number" :placeholder="depositMode === 'percent' ? '10' : '0'"
                        labelHidden :reserveErrorSpace="false" :disabled="depositMode === 'none'" :title="depositMode === 'none' ? 'select deposit mode from dropdown first' : 'deposit value'
                            " inputClass="w-full py-1" :error="depositError" />

                    <TheDropdown v-model="depositMode" input-class="py-1 py-1 min-w-32 sm:min-w-24"
                        :right-icon="ChevronUpDownIcon" :options="['none', 'fixed', 'percent']" />
                </div>
        ...
        <section class="min-w-0 py-4">
            <div class="mb-2 flex min-w-0 items-center justify-between gap-3">
                ...
                <TheTooltip text="Set to 0% to exclude VAT from the invoice." side="top" align="center"
                    class="hover:text-sky-600 dark:hover:text-emerald-400">
                    <InformationCircleIcon class="size-5 cursor-help" />
                </TheTooltip>
            </div>
            ...
            <div class="grid min-w-0 grid-cols-1 gap-2 sm:grid-cols-[minmax(0,1fr)_5.5rem]">
                <TheInput v-model="vatPercent" type="number" placeholder="20" labelHidden :reserveErrorSpace="false"
                    inputClass="w-full py-1" :error="inv.getFieldError('totals.vatRate')" />
