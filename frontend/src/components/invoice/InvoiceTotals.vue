...
<template>
    <div v-if="!invStore.invoice || !invStore.totals" class="text-base text-zinc-600 dark:text-zinc-400">
        No invoice loaded.
    </div>
    ...
    <div v-else class="min-w-0 space-y-4 text-sm">
        ...
        <div class="rounded-xl bg-zinc-50 px-3 py-3 dark:bg-zinc-900/40">
            ...
            <p class="mt-2 text-xs leading-5 text-zinc-500 dark:text-zinc-400">
                Deposits do not reduce balance due. Use the editor to create payment receipts for your clients.
            </p>
        </div>
    </div>
</template>
