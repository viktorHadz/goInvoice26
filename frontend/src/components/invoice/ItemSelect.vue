...
<template>
  <div class="relative col-span-8 grid grid-cols-subgrid items-center gap-6 py-5">
    ...
    <div class="absolute -bottom-1 w-full max-w-36">
      <span class="absolute top-0 left-1.5 h-4 w-2 rounded-bl-sm border-b border-l mask-t-from-0"></span>
      ...
      <span class="absolute top-0 -right-3 h-4 w-2 rounded-br-sm border-r border-b mask-t-from-0%"></span>
    </div>
    <div class="relative col-span-4 block py-1">
      ...
      <input id="combo-item-add-1" type="text"
        ...
        v-model="selectedItem.name" @input="searchQueries[itemType] = $event.target.value"
      ...
      <transition enter-active-class="transition duration-300 origin-top ease-out "
        ...
        leave-active-class="transition duration-150  ease-in" leave-from-class=" scale-y-100 origin-bottom opacity-100"
        ...
        <div v-if="filteredItems.length > 0 && dropDown"
          ...
          <div v-for="(item, index) in filteredItems" :key="item.id" :value="item">
            <li
              ...
              <button class="hover:text-acc dark:hover:text-acc flex cursor-pointer justify-between"
                @click="selectItem(item)">
                <div class="truncate">{{ item.name }}</div>
                <div class="truncate">{{ item.price }}</div>
              </button>
              <LeInput :id="`${item.id}-${index}`" class="placeholder:text-acc text-acc w-12" v-model="selectedItem.qty"
    ...
    <!-- Numerics -->
    <LeInput class="col-span-1" placeholder="x1" v-model="selectedItem.qty" :id="'invo-qty-1'" :type="'number'">
    </LeInput>
    <LeInput class="col-span-1" v-model="selectedItem.time" :disabled="itemType === 'style' ? true : false"
      :id="'invo-time-1'" :type="'number'" :placeholder="itemType === 'style' ? 'N/A' : 'min'"
      :title="itemType === 'style' ? 'No time property for styles' : 'min'"></LeInput>
    <LeInput class="col-span-1" v-model="selectedItem.price" :id="'invo-price-1'" :type="'number'"
      placeholder="item price"></LeInput>
    <input id="invo-total-1"
      class="text-acc placeholder:text-acc input hover:!ring-brdr col-span-1 cursor-default select-none"
      v-model="selectedItem.itemTotal" disabled="true" placeholder="0" title="Cannot edit item total price" />
  </div>
</template>
