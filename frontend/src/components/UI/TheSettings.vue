...
const shortcuts: ShortcutDefinition[] = [
  { key: 's', modifiers: ['ctrl', 'shift'], action: () => openSettings() },
]
...
<template>
  <TheTooltip v-if="showTrigger" side="bottom" align="end">
    <template #content>
  ...
  <Teleport to="body">
    <transition name="fade">
      <div v-if="settingsOpen" class="fixed inset-0 z-100 bg-black/45 backdrop-blur-[2px]"
        @click="() => closeSettings()" />
    </transition>

    <transition name="modal">
      <section v-if="settingsOpen"
        ...
        <header
          ...
          <DecorGradient />
          <div class="relative z-10 flex items-start justify-between gap-4 px-3 py-2 sm:px-5 sm:py-4">
            <div class="flex min-w-0 items-center gap-4">
              <div
                ...
                <Cog6ToothIcon class="stroke-1.5 size-6 text-sky-700 sm:size-7 dark:text-emerald-400" />
              </div>

              <div class="min-w-0">
                <div class="flex items-center gap-2">
                  <h2 class="text-xl font-semibold tracking-tight text-zinc-900 sm:text-2xl dark:text-zinc-100">
                    {{ title }}
                  </h2>
                </div>
                ...
                <p v-if="!canEditSettings" class="mt-1 text-xs font-medium text-amber-600 dark:text-amber-300">
                  {{ readOnlyNotice }}
                </p>
              </div>
            </div>
        ...
        <!-- Body -->
        <div class="grid min-h-0 flex-1 grid-cols-1 gap-0 lg:grid-cols-2" v-if="form">
          <!-- Left -->
          <div
            ...
            <div class="space-y-5 pb-10">
              ...
              <section :class="cardClass">
                <fieldset :disabled="!canEditSettings">
                  ...
                  <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <TheInput v-model="form.companyName" label="Company Name" :input-max-length="50"
                      placeholder="Your business name" autocomplete="name" :disabled="!canEditSettings" />

                    <TheInput v-model="form.email" label="Email" :input-max-length="50" placeholder="name@company.com"
                      autocomplete="email" :disabled="!canEditSettings" />

                    <TheInput v-model="form.phone" label="Phone" :input-max-length="20" placeholder="+44..."
                      autocomplete="tel" :disabled="!canEditSettings" />

                    <TheInput v-model="form.invoicePrefix" :input-max-length="50" label="Invoice prefix"
                      placeholder="INV-" :disabled="!canEditSettings" />
                  </div>

                  <div class="mt-4">
                    <label for="setts-c-addr" class="mb-1.5 block text-sm font-medium text-zinc-700 dark:text-zinc-300">
                      Company address
                    </label>
                    <textarea id="setts-c-addr" v-model="form.companyAddress" rows="3" maxlength="160"
                      class="input input-accent min-h-28 w-full resize-y rounded-lg px-3 py-2"
                      placeholder="Street&#10;City&#10;Postcode" :disabled="!canEditSettings" />
                  </div>
                </fieldset>
              </section>
              ...
              <section :class="cardClass">
                <fieldset :disabled="!canEditSettings">
                  ...
                  <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <TheDropdown v-model="form.currency" :options="currencyOptions" select-title="Currency"
                      :disabled="!canEditSettings" />

                    <TheDropdown v-model="form.dateFormat" :options="dateFormatOptions" select-title="Date format"
                      :disabled="!canEditSettings" />
                  </div>
                  ...
                  <div
                    ...
                    <div class="mb-2 flex items-center justify-between gap-2">
                      ...
                      <TheTooltip side="top" max-width-class="max-w-[320px]">
                        <template #content>
                    ...
                    <TheInput v-model="startingInvoiceNumberModel" type="number" labelHidden :reserveErrorSpace="false"
                      placeholder="1" inputClass="w-full"
                  ...
                  <label
                    ...
                    <span class="relative mt-0.5 shrink-0">
                      <input v-model="form.showItemTypeHeaders" type="checkbox" class="peer sr-only"
                        :disabled="!canEditSettings" />
                      <span
              ...
              <section :class="cardClass">
                <fieldset :disabled="!canEditSettings">
                  ...
                  <div class="space-y-4">
                    <div>
                      <label for="setts-payment-terms"
                        class="mb-1.5 block text-sm font-medium text-zinc-700 dark:text-zinc-300">
                        Payment terms
                      </label>
                      <textarea v-model="form.paymentTerms" id="setts-payment-terms" rows="3"
                        :maxlength="PAYMENT_TERMS_MAX" class="input input-accent w-full resize-y rounded-lg px-3 py-2"
                        placeholder="Payment terms shown on invoices" :disabled="!canEditSettings" />
                      <div class="mt-1 text-right text-xs" :class="form.paymentTerms.length > PAYMENT_TERMS_MAX * 0.9
                    ...
                    <div>
                      <label for="setts-pmnt-details"
                        class="mb-1.5 block text-sm font-medium text-zinc-700 dark:text-zinc-300">
                        Payment details
                      </label>
                      <textarea id="setts-pmnt-details" v-model="form.paymentDetails" rows="3"
                        :maxlength="PAYMENT_DETAILS_MAX" class="input input-accent w-full resize-y rounded-lg px-3 py-2"
                        placeholder="Bank transfer details, sort code, account number, IBAN, etc."
                        :disabled="!canEditSettings" />
                      <div class="mt-1 text-right text-xs" :class="form.paymentDetails.length > PAYMENT_DETAILS_MAX * 0.9
                          ? 'text-rose-600 dark:text-rose-300'
                    ...
                    <div>
                      <label for="setts-notes-footer"
                        class="mb-1.5 block text-sm font-medium text-zinc-700 dark:text-zinc-300">
                        Footer note
                      </label>
                      <textarea id="setts-notes-footer" v-model="form.notesFooter" rows="3" :maxlength="FOOTER_NOTE_MAX"
                        class="input input-accent w-full resize-y rounded-lg px-3 py-2"
                        placeholder="Optional footer or thank-you note" :disabled="!canEditSettings" />

                      <div class="mt-1 text-right text-xs" :class="form.notesFooter.length > FOOTER_NOTE_MAX * 0.9
                          ? 'text-rose-600 dark:text-rose-300'
            ...
            <div class="space-y-5 pb-10">
              ...
              <section :class="cardClass">
                ...
                <div
                  ...
                  <div v-if="hasLogo" class="flex w-full flex-col items-center gap-4">
                    <img :src="logoPreview!" alt="Invoice logo preview" class="max-h-36 rounded-lg object-contain" />
                    ...
                    <div class="flex flex-wrap justify-center gap-2">
                      <template v-if="canEditSettings">
                        ...
                        <TheButton type="button" @click="removeLogo" variant="danger" class="cursor-pointer">
                          Remove
                        </TheButton>
                      </template>
                      <p v-else class="text-xs text-zinc-600 dark:text-zinc-400">
                        {{ readOnlyNotice }}
                      </p>
                    </div>
                  </div>
              ...
              <section :class="cardClass">
                ...
                <SettingsPreview :form="form" :logo-preview="logoPreview" />
              </section>
            </div>
          </div>
        </div>
        ...
        <footer
          ...
          <div class="flex flex-wrap items-center justify-end gap-2">
            <TheButton type="button" @click="closeSettings" :variant="canEditSettings ? 'danger' : 'secondary'"
              class="cursor-pointer">
              {{ canEditSettings ? 'Cancel' : 'Close' }}
            </TheButton>
            <TheButton v-if="canEditSettings" type="button" @click="save" variant="success" class="cursor-pointer">
              Save settings
            </TheButton>
          </div>
        </footer>
