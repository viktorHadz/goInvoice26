...
<template>
  <div ref="menuRef" class="fixed top-3 right-4 z-50">
    <button type="button" :aria-expanded="open" aria-haspopup="menu"
      ...
      @click="toggleMenu">
      <UserAvatar :name="authStore.user?.name" :email="authStore.user?.email" :avatar-url="authStore.user?.avatarUrl"
      ...
      <section v-if="open"
        ...
        <header class="border-b border-zinc-300 px-4 py-3 dark:border-zinc-800">
          <div class="flex items-start gap-3">
            <UserAvatar :name="authStore.user?.name" :email="authStore.user?.email"
              :avatar-url="authStore.user?.avatarUrl" class="size-11 rounded-2xl" />
            ...
            <div class="min-w-0 flex-1">
              ...
              <p v-if="promoStatusLabel" class="text-mini mt-1 text-emerald-700 dark:text-emerald-400">
                {{ promoStatusLabel }}
              </p>
            </div>
          </div>
        ...
        <div class="space-y-4 p-4">
          <section>
            <div class="mb-2 text-[11px] font-semibold tracking-[0.16em] text-zinc-500 uppercase dark:text-zinc-400">
              Client Picker
            </div>

            <TheDropdown v-model="clientStore.selectedClient" :options="clientStore.clients"
              :disabled="!hasWorkspaceAccess || !clientStore.hasClients" :left-icon="UserIcon"
              :right-icon="ChevronUpDownIcon" label-key="name" value-key="id" :placeholder="!hasWorkspaceAccess
                ? 'Billing required'
            ...
            <p v-if="!hasWorkspaceAccess" class="mt-2 text-xs text-zinc-500 dark:text-zinc-400">
              Billing needs to be active before client and workspace tools are available.
            </p>

            <p v-else-if="!clientStore.hasClients" class="mt-2 text-xs text-zinc-500 dark:text-zinc-400">
          ...
          <section>
            ...
            <div class="grid grid-cols-1 gap-2">
              <button v-for="action in actionItems" :key="action.key" type="button"
    ...
    <TeamQuickMenu ref="teamMenuRef" :show-trigger="false" />
    <PlatformAccessQuickMenu ref="platformAccessMenuRef" :show-trigger="false" />
    <TheSettings ref="settingsRef" :show-trigger="false" />
  </div>
</template>
